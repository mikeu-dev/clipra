		...
		<div class="mx-auto mt-16 max-w-7xl">
			<div class="mb-10 flex w-full items-center justify-center gap-2">
				...
				>
					...
					<input
						type="search"
						placeholder={m.career_search_placeholder()}
						value={searchTerm}
			...
			{#if data.jobs.length === 0}
				<div class="py-12 text-center">
					<p class="text-muted-foreground">{m.career_empty_state()}</p>
				</div>
			{:else}
				<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
					{#each data.jobs as job (job.id)}
						<div
							class="group relative flex flex-col justify-between overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200 transition-all hover:shadow-md hover:ring-blue-500 dark:bg-slate-800 dark:ring-gray-700"
						>
							<div>
								<div class="flex items-center gap-x-4 text-xs">
									<time
										datetime={job.createdAt ? job.createdAt.toString() : ''}
										class="text-gray-500"
										>{job.createdAt ? new Date(job.createdAt).toLocaleDateString() : 'N/A'}</time
									>
									<a
										href={`/career?search=${job.type}`}
										class="relative z-10 rounded-full bg-blue-50 px-3 py-1.5 font-medium text-blue-600 hover:bg-blue-100"
										>{job.type}</a
									>
								</div>
							...
							<div
								class="mt-6 flex items-center justify-between border-t border-gray-100 pt-6 dark:border-gray-700"
							>
								<div class="flex items-center gap-2 text-sm text-gray-500">
									<Clock class="h-4 w-4" />
									{m.career_posted_label()}
								</div>
								<Button
									variant="ghost"
									size="sm"
