<script lang="ts">
	...
	let {
		...
		data: {
			positions: Position[];
			form: SuperValidated<Infer<PositionSchema>>;
		...
		untrack(() => data.form),
		{
			...
			onResult: ({ result }) => {
				if (result.type === 'success') {
					...
					toast.error(result.error?.message || 'Failed to create position');
				}
			}
		}
	);
	...
	const pages = {
		title: 'Positions',
		excerpt: 'Manage employee positions.'
	};
</script>
	...
	<div class="flex items-center justify-between gap-2">
		<div class="flex flex-col gap-1">
			...
			<p class="text-muted-foreground">
				You have {data?.positions.length} position(s)
			</p>
		</div>
		<div class="flex items-center gap-2">
			<Button onclick={() => (isOpen = true)} size="sm" variant="outline"
				><Plus /> Add Position</Button
			>
		</div>
	</div>
	<Separator />
	<DataTable data={data?.positions} columns={createColumns(data?.formDestroy, data.form)} />
</div>
...
<Dialog.Root bind:open={isOpen}>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title class="text-primary font-semibold">Add Position</Dialog.Title>
			<Dialog.Description>Create a new position for employees.</Dialog.Description>
		</Dialog.Header>
