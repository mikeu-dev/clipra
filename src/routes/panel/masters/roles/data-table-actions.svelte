...
<script lang="ts">
	...
	const deleteForm = superForm(
		untrack(() => formDestroy),
		{
			...
			id: untrack(() => `delete-form-${role.id}`),
			onResult: ({ result }) => {
...
<DropdownMenu.Root>
	...
	<DropdownMenu.Content align="end">
		<DropdownMenu.Group>
			...
			<DropdownMenu.Item>
				<a href="/panel/masters/roles/{role.id}/permissions" class="flex items-center">
					<Shield class="mr-2 size-4" /> Kelola Permissions
				</a>
			</DropdownMenu.Item>
...
<Dialog.Root bind:open={openEdit}>
	<Dialog.Content>
		<Dialog.Header>
			...
			<Dialog.Description>Update the role name and description.</Dialog.Description>
		</Dialog.Header>
