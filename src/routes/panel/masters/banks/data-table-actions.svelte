...
<script lang="ts">
	...
	const deleteForm = superForm(
		untrack(() => formDestroy),
		{
			...
			id: untrack(() => `delete-form-${bank.id}`),
			onResult: ({ result }) => {
	...
	const { form: deleteData, enhance: enhanceDelete, delayed: delayedDelete } = deleteForm;
	$deleteData.id = untrack(() => bank.id);
	...
	function handleOpenEdit() {
		$editData.id = bank.id;
		$editData.code = bank.code;
		$editData.name = bank.name || '';
		openEdit = true;
	}
</script>
...
<Dialog.Root bind:open={openEdit}>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>Edit Bank</Dialog.Title>
			<Dialog.Description>Update the bank details.</Dialog.Description>
		</Dialog.Header>
		<form method="POST" action="?/update" use:enhanceEdit>
			<input type="hidden" name="id" bind:value={$editData.id} />
			<Form.Field form={editForm} name="code">
				<Form.Control>
					{#snippet children({ props })}
						<Form.Label>Code</Form.Label>
						<Input {...props} bind:value={$editData.code} required />
					{/snippet}
				</Form.Control>
				<Form.FieldErrors />
			</Form.Field>
