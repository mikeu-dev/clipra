...
<script lang="ts">
	...
	let { formDestroy, id }: { formDestroy: any; id: any } = $props();
		...
		untrack(() => formDestroy),
		{
			...
			onResult: ({ result }) => {
				if (result.type === 'success') {
					toast.success(m.toast_success_stage_deleted());
					isOpenDestroy = false;
				} else if (result.type === 'error') {
					toast.error(result.error?.message || 'Failed to delete stage');
				}
			}
		}
	);
...
<Dialog.Root bind:open={isOpenDestroy}>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title class="text-primary font-semibold">Delete stage</Dialog.Title>
			<Dialog.Description>
