...
<script lang="ts">
	...
	let { formDestroy, id }: { formDestroy: SuperValidated<Infer<IdSchema>>; id: string } = $props();
		...
		untrack(() => formDestroy),
		{
			...
			onResult: ({ result }) => {
				if (result.type === 'success') {
					toast.success(m.toast_success_school_deleted());
					isOpenDestroy = false;
				} else if (result.type === 'error') {
					toast.error(result.error?.message || 'Failed to delete school');
				}
			}
		}
	);
...
<DropdownMenu.Root>
	...
	<DropdownMenu.Content>
		<DropdownMenu.Group>
			...
			<DropdownMenu.Item>
				<Button onclick={() => (isOpenDestroy = true)} size="sm" variant="ghost"
...
<Dialog.Root bind:open={isOpenDestroy}>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title class="text-primary font-semibold">Delete school</Dialog.Title>
			<Dialog.Description>
