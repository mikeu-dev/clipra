...
<script lang="ts">
	...
	const deleteForm = superForm(
		untrack(() => formDestroy),
		{
			...
			id: untrack(() => `delete-form-${religion.id}`),
			onResult: ({ result }) => {
	...
	const editForm = superForm(
		untrack(() => formEdit),
		{
			...
			id: untrack(() => `edit-form-${religion.id}`),
			onResult: ({ result }) => {
				if (result.type === 'success') {
	...
	function handleOpenEdit() {
		$editData.id = religion.id;
		$editData.name = religion.name ?? '';
		openEdit = true;
	}
</script>
...
<Dialog.Root bind:open={openEdit}>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>Edit Religion</Dialog.Title>
			<Dialog.Description>Update the religion name.</Dialog.Description>
		</Dialog.Header>
...
<Dialog.Root bind:open={openDelete}>
	<Dialog.Content>
		<Dialog.Header>
			...
			<Dialog.Description>
				This action cannot be undone. This will permanently delete the religion.
			</Dialog.Description>
		</Dialog.Header>
		<Dialog.Footer>
