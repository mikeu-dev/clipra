<script lang="ts">
		...
		untrack(() => data.form as SuperValidated<Infer<FormSchema>>),
		{
			...
			onResult: ({ result }) => {
				if (result.type === 'failure' || result.type === 'error') {
					...
					toast.success(m.toast_success_news_updated());
				}
			}
		}
	);
	...
	let thumbnailFiles: any[] = $state(
		$formData.thumbnail ? [{ source: $formData.thumbnail, options: { type: 'local' } }] : []
	);
...
<div class="hidden h-full flex-1 flex-col gap-8 p-8 md:flex">
	<div class="flex items-center justify-between gap-2">
		<div class="flex flex-col gap-1">
			<h2 class="text-3xl font-bold tracking-tight text-blue-900 dark:text-blue-100">Edit News</h2>
			<p class="text-muted-foreground">Edit existing article.</p>
		</div>
	</div>
	<Separator />
