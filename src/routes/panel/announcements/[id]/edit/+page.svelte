<script lang="ts">
		...
		untrack(() => data.form as SuperValidated<Infer<AnnouncementFormSchema>>),
		{
			...
			onResult: ({ result }) => {
				if (result.type === 'failure' || result.type === 'error') {
					toast.error(m.toast_error_announcement_updated());
				} else if (result.type === 'success' || result.type === 'redirect') {
					toast.success(m.toast_success_announcement_updated());
				}
			}
		}
	);
	...
	onMount(() => {
		if ($formData.targetType === 'role' && $formData.targetValue) {
			try {
				selectedRoles = JSON.parse($formData.targetValue);
			} catch {
				selectedRoles = [];
			}
		} else if ($formData.targetType === 'user' && $formData.targetValue) {
			try {
				selectedUsers = JSON.parse($formData.targetValue);
			} catch {
				selectedUsers = [];
			}
		}
	...
	const handleProcessFile = (_error: unknown, file: { serverId: string }) => {
		if (!_error && file?.serverId) {
			attachmentServerIds = [...attachmentServerIds, file.serverId];
		}
	};

	const handleRemoveFile = (_error: unknown, file: { serverId: string }) => {
		if (file?.serverId) {
	...
	<div class="flex items-center justify-between gap-2">
		<div class="flex items-center gap-3">
			...
			<div>
				...
				<p class="text-muted-foreground">Perbarui draf pengumuman Anda.</p>
			</div>
		</div>
	</div>
	<Separator />
	...
	<div class="max-w-7xl">
		<form method="POST" enctype="multipart/form-data" use:enhance class="grid gap-8 lg:grid-cols-3">
			...
			<div class="flex flex-col space-y-6">
				<Form.Field {form} name="status">
					...
					<Form.Description>
						{#if $formData.status === 'published'}
							Akan langsung dikirim ke semua target saat disimpan.
						{:else}
							Tetap disimpan sebagai draft.
						{/if}
					</Form.Description>
					<Form.FieldErrors />
				</Form.Field>
						...
						<div class="max-h-48 space-y-1 overflow-y-auto">
							{#each filteredUsers as user (user.id)}
								...
								>
									...
									{#if isSelected}
										<Badge variant="outline" class="border-blue-300 text-[10px] text-blue-600"
											>✓</Badge
										>
									{/if}
								</button>
				...
				<div>
					<span class="mb-2 block text-sm font-medium">Lampiran Baru (opsional)</span>
					<FilePond
				...
				>
					<Button variant="outline" href="/panel/announcements/{data.announcement.id}"
						>{m.btn_cancel()}</Button
					>
