...
<div class="hidden h-full flex-1 flex-col gap-8 p-8 md:flex">
	<div class="flex items-center justify-between gap-2">
		<div class="flex flex-col gap-1">
			...
			<p class="text-muted-foreground">Manage your quotations and orders.</p>
		</div>
		<div class="flex items-center gap-2">
			<Button href={localizeHref('/panel/crm/orders/create')} size="sm" variant="outline"
				><Plus class="mr-2 h-4 w-4" /> Create Quotation</Button
			>
		</div>
	</div>
	<Separator />
			...
			<Table.Body>
				{#each data.orders as order (order.id)}
					<Table.Row>
						...
						<Table.Cell>{new Date(order.createdAt || new Date()).toLocaleDateString()}</Table.Cell>
						...
						<Table.Cell class="text-right">
							<Button variant="ghost" size="sm" href={localizeHref(`/panel/crm/orders/${order.id}`)}
								>View</Button
							>
						</Table.Cell>
					</Table.Row>
				{:else}
					<Table.Row>
						<Table.Cell colspan={6} class="h-24 text-center">No sales orders found.</Table.Cell>
					</Table.Row>
				{/each}
			</Table.Body>
		</Table.Root>
