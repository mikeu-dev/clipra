		...
		untrack(() => data.form),
		{
			...
			onResult: ({ result }) => {
				if (result.type === 'failure' || result.type === 'error') {
					...
					toast.success(m.toast_success_vacancy_created());
				}
			}
		}
	);
	...
	<div class="bg-card text-card-foreground mx-auto w-full max-w-7xl rounded-lg border shadow-sm">
		<div class="flex flex-col space-y-1.5 p-6">
			...
			<p class="text-muted-foreground">
				Isi formulir berikut untuk memposting lowongan pekerjaan baru.
			</p>
		</div>
		<div class="p-6 pt-0">
			<form method="POST" use:enhance class="space-y-6">
				...
				<Form.Field {form} name="slug">
					<Form.Control>
						{#snippet children({ props })}
							<Form.Label>Slug (Opsional, otomatis dari judul)</Form.Label>
							<Input {...props} bind:value={$formData.slug} placeholder="frontend-developer" />
						{/snippet}
					</Form.Control>
					<Form.Description>Biarkan kosong untuk generate otomatis.</Form.Description>
					<Form.FieldErrors />
				</Form.Field>
